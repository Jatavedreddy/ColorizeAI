...
def handler_video(video_file: str | None, strength: float, frame_skip: int = 1, resolution: str = "Original", custom_width: int = None, custom_height: int = None):
...
"""
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Video"):
                ...
                frame_skip = gr.Slider(minimum=1, maximum=30, value=1, step=1, label="Frame Skip Interval (process every Nth frame)")
                resolution = gr.Dropdown(["Original", "720p", "1080p", "Custom"], value="Original", label="Output Resolution")
                custom_width = gr.Number(value=None, minimum=1, label="Custom Width (px)")
                custom_height = gr.Number(value=None, minimum=1, label="Custom Height (px)")
                run_vid = gr.Button("Colorize Video (ECCV16)")
                vid_out = gr.Video()

                def show_custom_fields(res):
                    return {custom_width: gr.update(visible=(res=="Custom")), custom_height: gr.update(visible=(res=="Custom"))}
                resolution.change(show_custom_fields, inputs=resolution, outputs=[custom_width, custom_height])

                run_vid.click(
                    handler_video,
                    inputs=[vid_input, strength_slider3, frame_skip, resolution, custom_width, custom_height],
                    outputs=vid_out,
                )
